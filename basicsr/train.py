...
from basicsr.data.data_sampler import EnlargedSampler, DistributedWeightedSampler
...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            if dataset_opt['sampler'] == "EnlargedSampler":
                train_sampler = EnlargedSampler(train_set, opt['world_size'],
                                                opt['rank'], dataset_enlarge_ratio)
            elif dataset_opt['sampler'] == "WeightedSampler":
                
                train_sampler = DistributedWeightedSampler(train_set, opt['world_size'],
                                                opt['rank'], dataset_enlarge_ratio)
            else:
                raise NotImplementedError("Unknown Dataset Sampler! ['EnlargedSampler' or 'WeightedSampler']")
...
def main():
    ...
    use_wandb = opt['logger']['wandb'].get('project', None)
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:     
            ...
            if dist.get_rank() == 0 and use_wandb is not None:
                train_logs = model.get_current_log()
                wandb.log({"Train":{ "l_pix":  train_logs['l_pix']}})
