...
parser.add_argument('--path_gt', default="./dataset_sample/JWST/gt/", type=str, help='Path to gt folder')
parser.add_argument('--path_lq', default="./dataset_sample/JWST/lq/", type=str, help='Path to lq folder')

parser.add_argument('--result_dir', default='./output/JWST_inference/', type=str, help='Directory for results')
...
result_dir_gt_npy = os.path.join(result_dir_tmp, "gt", 'npy')
...
os.makedirs(result_dir_gt_png, exist_ok=True)
...
os.makedirs(result_dir_gt_npy, exist_ok=True)
...
with torch.no_grad():
    for idx, (img_gt_path, img_lq_path) in enumerate(zip(images_gt, images_lq)):
        ...
        save_file_gt_png_path = os.path.join(result_dir_gt_png, f"{str(idx).zfill(5)}_gt.png")
        ...
        save_file_gt_npy_path = os.path.join(result_dir_gt_npy, f"{str(idx).zfill(5)}_gt.npy")
        ...
        # Saving npy images
        np.save(save_file_gt_npy_path, img_gt)
        ...
        cv2.imwrite(save_file_gt_png_path, img_as_ubyte(img_gt))
